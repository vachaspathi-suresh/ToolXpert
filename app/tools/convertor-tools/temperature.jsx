...
import { convertTemperature } from "../../../tool-functions/conversionTools";
...
const Temperature = () => {
  ...
  const startConvert = () => {
    if (fromValue.txtVal.trim() !== "" && fromValue.isValid) {
      let ans = convertTemperature(fromValue.value, from, to);
      setToValue(ans);
    } else {
  ...
  return (
    <SafeAreaView>
      ...
      <VStack marginY={30} marginX={2} space={2} alignItems={"center"}>
        <Text fontSize={25}>Temperature Convertor</Text>
        <VStack
          borderWidth={3}
        ...
        >
          ...
          >
            ...
            <Select.Item label="Reaumur" value="r" />
          </Select>
        </VStack>
